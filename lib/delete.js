...
function localSearch(changelog, replContext, next) {
  ...
  log.debug('entering delete localsearch wtih %j', changelog.object);
  ...
  localClient.search(targetDn, { scope: 'base' }, function(err, res) {
    if (err) {
      log.error('delete localsearch unable to search', err);
      throw new Error('unable to search for dn ' + targetDn, err);
    }
    ...
    res.on('error', function(err) {
      ...
      log.error('error while delete local searching', err);
      process.exit(1);
    });
    ...
    res.on('end', function(res) {
      ...
      if (!changelog.localEntry) {
        ...
        // filter does not match, sow we skip
        log.debug('%j exists but does not match locally, exiting',
                  changelog.object);
        // skip right to writing the checkpoint
        return common.writeCheckpoint(changelog, replContext, function() {
          return next(true);
  ...
  if (filter.matches(localEntry.object)) {
    ...
    localClient.del(localEntry.dn, function(err) {
      ...
      if (err) {
        ...
        throw new Error('unable to delete entry '+ localEntry.dn, err);
      }
