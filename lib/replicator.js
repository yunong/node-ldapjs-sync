...
/**
 * This object contains all of the context neccessary for a local server to
 * replicate against a remote ldap server. An 'init' even is emmited on
 ...
 *  replSuffix : the root dn where the replicated entries are stored on the
 *  slave. (string)
...
function Replicator(options) {
  ...
  if (typeof(options.remoteUrl) !== 'string') {
    throw new TypeError('options.remoteUrl (string) required');
  }
  if (!options.remoteClientCfg) {
    ...
    if (typeof(options.remoteClientCfg) !== 'object') {
      ...
      options.remoteClientCfg.url = options.remoteUrl;
    }
  }
  ...
  this.remoteUrl = ldapjs.url.parse(options.remoteUrl, true);
  this.localUrl = ldapjs.url.parse(options.localUrl, true);
  this.replSuffix = options.replSuffix;
  this.log.debug('remote url', this.remoteUrl);
  this.log.debug('remote DN', this.remoteUrl.DN.rdns.toString());
    ...
    self.remotePool = poolModule.Pool({
      ...
      create: function(callback) {
        ...
        remoteClient.once('connect', function(err) {
          ...
          if (self.remoteUrl.auth) {
            common.bindClient(self.remoteUrl.auth, remoteClient, self.log,
                              function(err) {
              if (err) {
